...
class Task(WorkflowTask):
    ...
    def execute(self, github_body, settings) -> bool:
        ...
        if (
            ...
            "check_run.requested_action" and
            github_body.get("requested_action", {}).get("identifier", "") ==
            "override"
        ):
            self.actions = None
            self.pass_summary = (
        ...
        status = (
            ...
            (int(settings.get("max_changed_files", "0")) or float("inf")) and
            github_body.get("pull_request", {}).get("additions", 0) <=
            (int(settings.get("max_additions", "0")) or float("inf")) and
            github_body.get("pull_request", {}).get("deletions", 0) <=
